...
class cart extends base {
	...
	public function cartheader(){
		...
		if($shopids!=NULL){
			...
			$shoplist=$this->db->GetList("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid where `id` in($shopids) limit 5",array("key"=>"id"));
	...
	public function cartshop(){		
		...
		if($shopids!=NULL){
			...
			$shoplist=$this->db->GetList("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid where `id` in($shopids) limit 7",array("key"=>"id"));
	...
	public function cartlist(){	
		...
		if($shopids!=NULL){
			...
			$shoplist=$this->db->GetList("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid where `id` in($shopids)",array("key"=>"id"));
	...
	public function Fastpay(){
		...
		if($_GET['shopid']){
			...
			$shoplist=$this->db->GetOne("SELECT zongrenshu,canyurenshu,yunjiage as price, title as cg_title FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid  where `id` = $id");
