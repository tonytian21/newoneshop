...
HTML;
	...
	public function recharge(){
		...
		if(empty($wheres)){
			...
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到云购账户' or `content`='使用余额宝充值到云购账户' OR `content`='通过微信公众号充值' or `content`='中奖充值') AND `type`='1'"); 

			$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到云购账户' or `content`='使用余额宝充值到云购账户' OR `content`='通过微信公众号充值'or `content`='中奖充值') AND `type`='1'"); 
		...
		if(empty($wheres)){
			...
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到云购账户' or `content`='使用余额宝充值到云购账户' OR `content`='通过微信公众号充值'or `content`='中奖充值') AND `type`='1' order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
