...
class shopajax extends base
{
    ...
    public function getUserBuyList()
    {
        ...
        if ($state == - 1) {
            // 参与云购的商品 全部...
            $shoplist = $this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where a.uid='$member[uid]' GROUP BY shopid ");
            
            $shoplistall['listItems'] = $this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where a.uid='$member[uid]' GROUP BY shopid order by a.time desc limit $FIdx,$EIdx ");
        } elseif ($state == 1) {
            // 参与云购的商品 进行中...
            $shoplist = $this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where a.uid='$member[uid]' and b.q_end_time is null GROUP BY shopid  ");
            
            $shoplistall['listItems'] = $this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where a.uid='$member[uid]' and b.q_end_time is null GROUP BY shopid order by a.time desc limit $FIdx,$EIdx ");
        } else {
            // 参与云购的商品 已揭晓...
            $shoplist = $this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id left join `@#_shoplist_term` c on b.gid=c.id where a.uid='$member[uid]' and b.q_end_time is not null GROUP BY shopid ");
            
            $shoplistall['listItems'] = $this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id left join `@#_shoplist_term` c on b.gid=c.id where a.uid='$member[uid]' and b.q_end_time is not null GROUP BY shopid order by a.time desc limit $FIdx,$EIdx ");
        }
    ...
    public function getUserOrderList()
    {
        ...
        $member = $this->userinfo;
        $order = $this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where b.q_uid='$member[uid]' ");
        
        $orderlist['listItems'] = $this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where b.q_uid='$member[uid]' and a.huode!=0 order by a.time desc limit $FIdx,$EIdx ");
    ...
    public function getUserPostList()
    {
        ...
        $member = $this->userinfo;
        $post = $this->db->GetList("select * from `@#_shaidan` a left join `@#_shoplist_term` b on a.sd_shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where a.sd_userid='$member[uid]' order by a.sd_time desc");
        
        $postlist['listItems'] = $this->db->GetList("select * from `@#_shaidan` a left join `@#_shoplist_term` b on a.sd_shopid=b.id left join `@#_shoplist` c on c.gid=b.id where a.sd_userid='$member[uid]' order by a.sd_time desc limit $FIdx,$EIdx");
    ...
    public function getUserUnPostList()
    {
        ...
        $member = $this->userinfo;
        // 获得的商品
        $orderlist = $this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where b.q_uid='$member[uid]' order by a.time desc");
        
        // 获取晒单
        $postlist = $this->db->GetList("select * from `@#_shaidan` a left join `@#_shoplist_term` b on a.sd_shopid=b.id left join `@#_shoplist` c on b.sid=c.gid where a.sd_userid='$member[uid]' order by a.sd_time desc");
        $huoid = '';
        ...
        // 未晒单
        $unpost = $this->db->GetList("select * from  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  where id in($rd_id) order by id");
        
        $unpostlist['listItems'] = $this->db->GetList("select * from  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  where id in($rd_id) order by id limit  $FIdx, $EIdx");
