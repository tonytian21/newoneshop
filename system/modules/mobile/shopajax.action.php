...
class shopajax extends base
{
    ...
    public function getUserOrderList()
    {
        ...
        $member = $this->userinfo;
        $order = $this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid left join `@#_shoplist_en` sen on sen.egid=c.gid  where b.q_uid='$member[uid]' ");
        
        $orderlist['listItems'] = $this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid left join `@#_shoplist_en` sen on sen.egid=c.gid  where b.q_uid='$member[uid]' and a.huode!=0 order by a.time desc limit $FIdx,$EIdx ");
    ...
    public function getUserPostList()
    {
        ...
        $member = $this->userinfo;
        $post = $this->db->GetList("select * from `@#_shaidan` a left join `@#_shoplist_term` b on a.sd_shopid=b.id left join `@#_shoplist` c on b.sid=c.gid left join `@#_shoplist_en` sen on sen.egid=c.gid  where a.sd_userid='$member[uid]' order by a.sd_time desc");
        
        $postlist['listItems'] = $this->db->GetList("select * from `@#_shaidan` a left join `@#_shoplist_term` b on a.sd_shopid=b.id left join `@#_shoplist` c on c.gid=b.sid left join `@#_shoplist_en` sen on sen.egid=c.gid  where a.sd_userid='$member[uid]' order by a.sd_time desc limit $FIdx,$EIdx");
    ...
    public function getUserUnPostList()
    {
        ...
        $member = $this->userinfo;
        // 获得的商品
        $orderlist = $this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist_term` b on a.shopid=b.id left join `@#_shoplist` c on b.sid=c.gid left join `@#_shoplist_en` sen on sen.egid=c.gid  where b.q_uid='$member[uid]' order by a.time desc");
        
        // 获取晒单
        $postlist = $this->db->GetList("select * from `@#_shaidan` a left join `@#_shoplist_term` b on a.sd_shopid=b.id left join `@#_shoplist` c on b.sid=c.gid left join `@#_shoplist_en` sen on sen.egid=c.gid  where a.sd_userid='$member[uid]' order by a.sd_time desc");
        $huoid = '';
        ...
        // 未晒单
        $unpost = $this->db->GetList("select * from  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid   where id in($rd_id) order by id");
        
        $unpostlist['listItems'] = $this->db->GetList("select * from  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid  where id in($rd_id) order by id limit  $FIdx, $EIdx");
