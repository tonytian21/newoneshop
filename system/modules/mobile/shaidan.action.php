...
class shaidan extends base {
	...
	public function detail(){
		...
		if(!empty($shaidan['sd_shopid'])){
			$goods = $this->db->GetOne("select * from `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid  where `id` = '$shaidan[sd_shopid]' order by `qishu` DESC");
		}else{
