...
class home extends base
{
            ...
            if (! $this->userinfo) {
                if (! $uid && ! $_GET['wxid']) {
                    if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
                        header("Location: " . WEB_PATH . "/api/wxlogin");
                        exit();
                    }
                }
                if (! isset($_GET['wxid'])) {
    ...
    public function postsingle()
    {
        ...
        if ($_POST) {
            ...
            for ($i = 0; $i < $num; $i ++) {
                ...
                $img[$i] = str_replace('/uploads/', '', $img[$i]);
                $pic .= trim($img[$i]) . ";";
            }
    ...
    {
        $uploadDir = $_SERVER['DOCUMENT_ROOT'] . '/uploads/shaidan/' . date('Ymd', time()) . '/';
        if (! is_dir($uploadDir)) {
    ...
    {
        $fileurl = $_SERVER['DOCUMENT_ROOT'] . '/uploads/shaidan/' . date('Ymd', time()) . '/' . $_POST['filename'];
