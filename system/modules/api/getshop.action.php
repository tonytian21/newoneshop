...
class getshop extends SystemAction
{
    ...
    public function lottery_shop_json()
    {
        ...
        echo json_encode(array(
            ...
            "user" => "$user",
            "zongrenshu" => $info['zongrenshu'],
            "q_user_code" => $info['q_user_code'],
            "qishu" => $info['qishu'],
            "upload" => $upload,
            "thumb" => $info['thumb'],
            "id" => $info['id'],
 ...
 //
    ...
    public function lottery_shop_getjson()
    {
        if (isset($_POST['gid'])) {
            ...
            $infos = $db->GetOne("SELECT * FROM `@#_member` where `uid` ='$info[uid]'");
            if ($extinfo['username'] != '') {
                $info['user'] = $extinfo['username'];
            } elseif ($infos['mobile'] != '') {
                $info['user'] = substr_replace($infos['mobile'], '****', 3, 4);
            } else {
                $info['user'] = substr_replace($infos['email'], '****', 3, 4);
            }
