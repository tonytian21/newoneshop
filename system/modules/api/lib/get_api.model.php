		...
		*/
		...
		if(!isset($args[1])){
			...
			if(!isset($args[2])){
				...
				$info = $this->GetOne("SELECT * FROM `@#_position_data` where `pos_id` = '$pos_info[pos_id]' LIMIT 1");				
				$info = $this->GetOne("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid   where `id` = '$info[con_id]' LIMIT 1");	
