...
function get_shop_if_jiexiao($shopid=null){
	...
	$record=$db->GetOne("select * from `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid where `id`='$shopid' and `q_showtime` = 'N' LIMIT 1");
