 ...
 // 模块文件名
    ...
    public function buyshop($id, $shopinfo, $member)
    {
        ...
        if ($shopinfo['zongrenshu'] > $shopinfo['canyurenshu']) {
            ...
            if ($autoadd == 1) {
                // 需要进入下一期 #添加下一期的商品ID值
                $shoptem = $this->db->GetOne("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid WHERE `id` = '$shopid'  LIMIT 1");
                $nextshopsid = $shoptem['sid'];
                $nextshopinfo = $this->db->GetOne("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid  WHERE `sid` = '$nextshopsid' ORDER BY  `qishu` DESC LIMIT 1");
            ...
            if (is_array($member)) {
                ...
                if ($mshop == 1) {
                    ...
                    for ($i = 0; $i < count($shopid); $i ++) {
                        $id = $shopid[$i];
                        // 查询商品信息
                        $shopinfo = $this->db->GetOne("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid  WHERE `id` = '$id' LIMIT 1");
