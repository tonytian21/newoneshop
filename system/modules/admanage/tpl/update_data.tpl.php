	...
	<form name="myform" action="" method="post" enctype="multipart/form-data">
		<table width="100%" class="lr10">
			...
			<tr>
				...
				<td>
					<select name="type" class="selecttype">
						...
						<option <?php echo $adlist[0]['type']=="couplet"?'selected="selected"':'';?> value="couplet">对联广告</option>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr <?php if ($adlist[0]['type']=="img"){
				...
				echo 'class="adtext"';
			}elseif($adlist[0]['type']=="code"){
				echo 'class="adcode"';
			}elseif($adlist[0]['type']=="couplet"){
				echo 'class="couplet_img"';
			}?>>
				...
				<td>
					...
					<?php }elseif($adlist[0]['type']=="couplet"){
						$ad_img = explode(',',$adlist[0]['content']);
						?>
						<input type="text" value="<?php echo $ad_img[0];?>" name="couplet_left" id="imagetext1" class="input-text wid300">
						<input type="button" class="button"
							   onClick="GetUploadify('<?php echo WEB_PATH; ?>','uploadify','缩略图上传','image','admanage',1,500000,'imagetext1')"
							   value="上传图片"/>
						<img style="height:50px; margin-left:100px;" src="<?php echo G_UPLOAD_PATH.'/'.$ad_img[0];?>" />
						<br/>
						<input type="text" value="<?php echo $ad_img[1];?>" name="couplet_right" id="imagetext2" class="input-text wid300">
						<input type="button" class="button"
							   onClick="GetUploadify('<?php echo WEB_PATH; ?>','uploadify','缩略图上传','image','admanage',1,500000,'imagetext2')"
							   value="上传图片"/>
						<img style="height:50px; margin-left:100px;" src="<?php echo G_UPLOAD_PATH.'/'.$ad_img[1] ;?>" />
					<?php }else{ ?>
