...
class content extends admin
{
    ...
    public function __construct()
    {
        ...
        $this->categorys = $this->db->GetList("SELECT * FROM `@#_category` WHERE 1 order by `parentid` ASC,`cateid` ASC", array(
            'key' => 'cateid'
        ));
        
        $this->models = $this->db->GetList("SELECT * FROM `@#_model` WHERE 1", array(
            'key' => 'modelid'
        ));
    }
    ...
    public function article_edit()
    {
        ...
        $this->ment = array(
            ...
            array(
                "lists",
                "内容管理",
                ROUTE_M . '/' . ROUTE_C . "/article_list"
            ),
            
            array(
                "insert",
                "添加文章",
                ROUTE_M . '/' . ROUTE_C . "/article_add"
            )
    ...
    public function article_list()
    {
        ...
        $articlelist = $this->db->GetPage("SELECT * FROM `@#_article` WHERE $list_where order by `order` DESC", array(
            "num" => $num,
            "page" => $pagenum,
            "type" => 1,
            "cache" => 0
...
HTML;
    ...
    public function goods_list()
    {
        $this->ment = array(
            ...
            array(
                "lists",
                "商品管理",
                ROUTE_M . '/' . ROUTE_C . "/goods_list"
            ),
            
            array(
                "add",
                "添加商品",
                ROUTE_M . '/' . ROUTE_C . "/goods_add"
            ),
            
            array(
                "renqi",
                "人气商品",
                ROUTE_M . '/' . ROUTE_C . "/goods_list/renqi"
            ),
            
            array(
                "xsjx",
                "限时揭晓商品",
                ROUTE_M . '/' . ROUTE_C . "/goods_list/xianshi"
            ),
            
            array(
                "qishu",
                "期数倒序",
                ROUTE_M . '/' . ROUTE_C . "/goods_list/qishu"
            ),
            
            array(
                "danjia",
                "单价倒序",
                ROUTE_M . '/' . ROUTE_C . "/goods_list/danjia"
            ),
            
            array(
                "money",
                "商品价格倒序",
                ROUTE_M . '/' . ROUTE_C . "/goods_list/money"
            ),
            
            array(
                "money",
                "已揭晓",
                ROUTE_M . '/' . ROUTE_C . "/goods_list/jiexiaook"
            ),
            
            array(
                "money",
                "<font color='#f00'>期数已满商品</font>",
                ROUTE_M . '/' . ROUTE_C . "/goods_list/maxqishu"
            )
        ...
        if ($cateid) {
            ...
            if ($cateid == 'maxqishu') {
                ...
                $list_where = "`qishu` = `maxqishu` and `q_end_time` is not null and `q_uid` is null";
            }
    ...
    public function goods_go_one()
    {
        ...
        $total_user = 0;
        if ($post_userdisp <= 0) {
            $go_list = $this->db->GetPage("select * from `@#_member_go_record` where `shopid` = '$gid' order by `id` desc", array(
                "num" => 20,
                "page" => $pagenum,
                "type" => 1,
                "cache" => 0
            ...
            $go_list = $this->db->GetPage("select * from `@#_member_go_record` where `shopid` = '$gid' and `uid`='$post_userdisp' order by `id` desc", array(
                "num" => 20,
                "page" => $pagenum,
                "type" => 1,
                "cache" => 0
    ...
    {   
        ...
        $BrandList = $this->db->GetList("SELECT * FROM `@#_brand` where 1", array(
            "key" => 'id'
        ));
        
        $categorys = $this->db->GetList("SELECT * FROM `@#_category` WHERE `model` = '1' order by `parentid` ASC,`cateid` ASC", array(
            'key' => 'cateid'
        ));
    ...
    public function goods_add()
    {
        ...
        if ($cateid) {
            ...
            $BrandList = $this->db->GetList("SELECT * FROM `@#_brand` where `cateid`='$cateid'", array(
                "key" => "id"
            ));
        } else {
