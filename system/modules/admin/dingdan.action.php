...
class dingdan extends admin
{
    ...
    public function __construct()
    {
        ...
        $this->ment = array(
            ...
            array(
                "lists",
                "订单列表",
                ROUTE_M . '/' . ROUTE_C . "/lists"
            ),
            
            array(
                "lists",
                "中奖订单",
                ROUTE_M . '/' . ROUTE_C . "/lists/zj"
            ),
            
            array(
                "lists",
                "已发货",
                ROUTE_M . '/' . ROUTE_C . "/lists/sendok"
            ),
            
            array(
                "lists",
                "未发货",
                ROUTE_M . '/' . ROUTE_C . "/lists/notsend"
            ),
            
            array(
                "insert",
                "已完成",
                ROUTE_M . '/' . ROUTE_C . "/lists/ok"
            ),
            
            array(
                "insert",
                "已作废",
                ROUTE_M . '/' . ROUTE_C . "/lists/del"
            ),
    ...
    public function lists()
    {
        ...
        $recordlist = $this->db->GetPage("SELECT * FROM `@#_member_go_record` $list_where", array(
            "num" => $num,
            "page" => $pagenum,
            "type" => 1,
            "cache" => 0
    ...
    private function send_wx_ship_code($gid = null)
    {
        ...
        $info = $this->db->GetOne("SELECT * FROM `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid  WHERE `id` = '$gid'");
