...
final class my_db_mysqli {
	...
	*/
	...
	public function free_result() {	
		if($this->lastresult && is_object($this->lastresult)) $this->lastresult->free();	
	}
...
class model {
	...
	public function __construct() {
		...
		$this->db = my_db_mysqli::GetObject($this->db_config[$this->db_setting]);
	}
...
//		$postStr = $GLOBALS["HTTP_RAW_POST_DATA"];
		...
		if($keyword == 'debug'){
			...
			$ret = $this->db -> GetList("SELECT * FROM  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid WHERE `q_user` = '' AND `qishu` < 5 ORDER BY `time`  DESC LIMIT 0 , 5");
			...
			$ret = $this->db -> GetList("SELECT * FROM  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid WHERE `q_user` = '' AND `renqi` = 1 ORDER BY `time` DESC LIMIT 0 , 5");
			$ArticleCount = count($ret);
			if($ArticleCount >= 1){
				foreach($ret as $v){
					$v['thumbnail_pic'] = $img_url .'/'. $v['thumb'];
			...
			$ret = $this->db -> GetList("SELECT * FROM  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid WHERE `q_user` = '' AND `pos` = 1 ORDER BY `time` DESC LIMIT 0 , 5");
			$ArticleCount = count($ret);
			if($ArticleCount >= 1){
				foreach($ret as $v){
					$v['thumbnail_pic'] = $img_url .'/'. $v['thumb'];
			...
			$goods_name = $keyword;
			$ret = $this->db -> GetList("SELECT * FROM  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid  left join `@#_shoplist_en` sen on sen.egid=A.gid WHERE  `title` LIKE '%$goods_name%' AND `q_user` = '' LIMIT 0,5");
			$ArticleCount = count($ret);
			if($ArticleCount >= 1){
				foreach($ret as $v){
					$v['thumbnail_pic'] = $img_url .'/'. $v['thumb'];
	...
	protected function plusTj($base_url,$fromUsername){
		$ret = $this->db -> GetList("SELECT * FROM  `@#_shoplist` A inner join `@#_shoplist_term` B on A.gid=B.sid left join `@#_shoplist_en` sen on sen.egid=A.gid  WHERE  `q_user` = ''");
		$tj_count = count($ret);
