...
<!-- 内页顶部 -->
    ...
    <section class="goodsCon" style="position: relative;top: 49px;">
	    ...
	    <div class="goodsNav">
            <ul id="divGoodsNav">
                ...
                <li order="">
					...
					<dd type="0" class="sOrange"><a href="javascript:;"><img style="width: 30px;height: 30px;border-radius: 3px;float: left;margin:10px 5px 0 0;" src="<?php echo G_TEMPLATES_IMAGE; ?>/mobile/moren.png" alt=""><span>默认排序</span></a></dd>
						<?php $data=$this->DB()->GetList("select * from `@#_category` where `model`='1' order by `order` desc",array("type"=>1,"key"=>'',"cache"=>0)); ?>
						<?php $ln=1;if(is_array($data)) foreach($data AS $categoryx): ?>
						<dd><a href="javascript:;"><img style="width: 30px;height: 30px;border-radius: 3px;float: left;margin:10px 5px 0 0;" src="<?php echo G_UPLOAD_PATH; ?>/<?php echo $categoryx['pic_url']; ?>"><span  id="<?php echo $categoryx['cateid']; ?>"><?php echo $categoryx['name']; ?></span></a></dd>
        ...
        <!-- 列表 -->
       <div class="goodsList" style="top: 49px;">
