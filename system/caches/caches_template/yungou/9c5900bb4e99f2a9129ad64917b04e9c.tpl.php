...
<div class="shop_process">
	<ul class="process">
		...
		<li class="arrow_1"></li>
		<li class="secend_step">第二步：乐币支付</li>
		<li class="arrow_2"></li>
	...
	<div class="submitted">
		<ul class="order">
			...
			<li class="end" id="shoplist<?php echo $shops['id']; ?>">
				...
				<span class="moneys"><?php echo $shops['money']; ?>乐购币</span>
				<span class="money"><span class="color"><b><?php echo $shops['yunjiage']; ?>乐购币</b></span></span>
				<span class="num">
				<a href="JavaScript:;" val="<?php echo $shops['id']; ?>" class="jia">+</a>				
                    ...
                    <p class="message"></p>
                    			 <a href="javascript:;" class="shop_last_all" val="<?php echo $shops['id']; ?>">购买全部</a>  
				</span>
			...
			<li class="ts">
				<p class="right">总计:<span id="moenyCount"  style="color: #f60;"><?php echo $MoenyCount; ?></span>乐币</p>
			</li>
		</ul>
	</div>
	<h5>
		<a href="<?php echo WEB_PATH; ?>" id="but_on">返回继续乐购</a>
		<input id="but_ok" type="button" value="去结算"  name="submit"/>
	</h5>
</div>  
...
num.keyup(function(){
	...
	if($(this).val() > info[shopid]['shenyu']){
		message.eq(zindex).text("乐购次数不能超过"+info[shopid]['shenyu']+"次");		
		$(this).val(info[shopid]['shenyu']);
		UpdataMoney(shopid,$(this).val(),zindex);		
		return;
	}
	if($(this).val()<1){
		message.eq(zindex).text("乐购次数不能少于1次");
		$(this).val(1);
		UpdataMoney(shopid,$(this).val(),zindex);
		return;
	}	
