...
class sendmobile {
	...
	**/	
	...
	private function cfg_send_2(){
		$mobile = $this->mobile['cfg_mobile_2'];
		$config = $this->config;
		preg_match_all('/(\d{1,8}(\.\d+)?)/is',$config['content'],$matches);
		$code = $matches[0][0];
		$nr ="您的验证码是：{$code}。请不要把验证码泄露给其他人。";
		$url = "http://www.smsbao.com/sms?u=".$mobile["mid"]."&p=".md5($mobile["mpass"])."&m=".$config["mobile"]."&c={$nr}";
		$ch2 = curl_init($url);
