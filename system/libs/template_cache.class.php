	...
	private function template_parse(){	
		...
		$this->content = preg_replace_callback ("/\{$stag:(\\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)([\$a-zA-Z_0-9\[\]\'\"\$\x7f-\xff]+)\}/s",
										function($vars){
											...
											return $this->addquote('<?php echo '.$vars[1].$vars[2].'; ?>');
										},$this->content);
		
										
		$this->content = preg_replace_callback ("/\{$stag:([a-zA-Z_0-9\[\]\'\"\$\x7f-\xff\+\-\*\/]+)\}/s",
										function($vars){
											return $this->addquote('<?php echo '.$vars[1].'; ?>');
										},$this->content);
		...
		//保留标签标签
		$this->content = preg_replace_callback ( "/\{$stag:(\w+)\s+([^}]+)\}/i", function($vars){
										return self::pc_tag_html($vars[1],$vars[2],$vars[0]);
								}, $this->content);
		
		
		
		//echo $this->content; 
		$this->content = preg_replace_callback ( "/\{$stag:(\w+):$etag\}/i",function($vars){
										return self::end_html_tag($vars[1],$vars[0]);
									} ,$this->content);
		
		
		//模块调用标签		
		$this->content = preg_replace_callback ( "/\{$stag:m=(\w+) mod=[\"|\'](\w+)[\"|\']([^}]+)\}/i", function($vars){
										return self::pc_module_tag($vars[1],$vars[2],$vars[3]);
								} , $this->content);		
		$this->content = preg_replace_callback ( "/\{$stag:m=(\w+):$etag\}/i", function($vars){
										return self::end_module_tag($vars[1],$vars[0]);
								},$this->content);
